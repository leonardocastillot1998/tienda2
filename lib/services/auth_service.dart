...
class AuthService {
  ...
  /// Removes all history entries for the given user from shared preferences.
  Future<void> clearHistory(String username) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      await prefs.remove('history_\$username');
    } catch (e) {
  ...
  Future<void> addCustomProduct({
    ...
    try {
      ...
      productWithId['id'] ??= DateTime.now().microsecondsSinceEpoch.toString();
      list.insert(0, productWithId);
      await prefs.setString(key, json.encode(list));
    } catch (e) {
  ...
  Future<void> updateCustomProduct({
    ...
    try {
      ...
      updated['id'] ??=
          current['id'] ?? DateTime.now().microsecondsSinceEpoch.toString();
      list[index] = updated;

      await prefs.setString(key, json.encode(list));
    } catch (e) {
      debugPrint('Failed to update custom product: $e');
    }
  }

  Future<void> deleteCustomProduct({
    required String username,
    required int index,
  }) async {
    try {
      ...
      debugPrint('Failed to delete custom product: $e');
    }
  }
  ...
  static Map<String, dynamic>? calculateClosestRewardProgress(
    ...
    final progressPercentage = canRedeem
        ...
        : (pointsRequired <= 0
              ? 0.0
              : (currentPoints / pointsRequired * 100)
                    .clamp(0, 100)
                    .toDouble());
    ...
    return {
      ...
      'statusLabel': hasRedeemableReward
          ? 'Listo para canjear!'
  ...
  Future<Map<String, dynamic>?> getClosestRewardProgress(
    int currentPoints,
  ) async {
    try {
      final products = await _supabase.from('productos').select();
      final typedProducts = products.whereType<Map<String, dynamic>>().toList();
